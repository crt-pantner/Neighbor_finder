...
def get_arguments():
    ...
    parser.add_argument("-choords", required=False, type=int, help="Flanking region around proteins. If no value is passed, will pick proteins from whole chromosome", default=0)
    parser.add_argument("-o", required=False, type=str, help="Prefix for output csv file containing the proteins and their pairs. Default is 'output_pair.csv'", default="output_pairs")
...
def logger(args):
    level=logging.basicConfig(level=logging.DEBUG if args==True else logging.WARNING, format="%(message)s")
...
def get_pairs(protein, db, aegerolysin_proteins, choords):
    ...
    if choords == 0:
        ...
        region = db.region(seqid=feature.seqid, start=start, end=end, featuretype="gene")
    try:
        pairs = []
...
def output_pairs(protein, pairs, output_file):
    ...
    output.extend([protein.short_name, protein.protid, protein.feature.start, protein.feature.end])
    ...
    for group in tqdm(organism_groups, desc="Processing groups"):
        try:
            ...
            for protein in organism_groups[group]["proteins"]:
                ...
                pairs = get_pairs(db=db, aegerolysin_proteins=aegerolysin_seq_dict, protein=protein, choords=arguments.choords)
